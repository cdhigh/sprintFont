...
### 2.5 自动布线（实验性功能）
Sprint-Layout对初学者和业余爱好者（像我这样的）来说太好用了，不用画电路图，不用学习一大堆的概念名词，比如网表，鼠线，设计约束什么一大堆，就简简单单的放置几个元件，连上线就好了，什么EMC，什么阻抗匹配，什么什么的...，跟我们有什么关系，我就想用单片机点亮几个数码管，控制几个继电器而已，让我安静的吃完盒饭不好嘛～～～

这个插件尝试更进一步，给Sprint-Layout增加自动布线功能，方案和Kicad一样：先导出为DSN文件，使用开源自动布线软件freerouting进行自动布线并将自动布线结果保存为SES文件，最后将SES导入Sprint-Layout。
![autorouter_main](docs/pic/autorouter_main.png)



#### 2.5.1 设计考虑
* 自动布线要达到较好的质量，需要很多适当定义的约束，但是规则太复杂就对初学者和简单的项目不友好了，而且Sprint-Layout也没有提供封装和网表信息，所以权衡之下，电路板布线整板使用同一个规则，并且规则尽可能的简化，目前仅提供 [布线宽度，过孔外径，过孔内径，孔到孔间隙，贴片到贴片间隙] 这5个规则设置项，如果需要更多的设置项，也可以自己修改生成的DSN文件。
* 电源线需要更粗的走线怎么办？预先布好电源线就好，同样的，如果有一些关键的线，比如时钟线之类的也可以预先布好，再让freerouting布剩下的线。




#### 2.5.2 各种功能细节说明
* 仅支持单面板和双面版，默认为双面板，如果仅需要单面走线，可以在freerouting的菜单“参数”|“自动布线”里面选择需要布线的板层
...
* 如果部分区域不希望freerouting走线，则可以画一个多边形，然后设置为“禁止区”。
